...
function hook_library() {
  ...
  $libraries['library-1'] = array(
    ...
    'version' => '1.2',
    'js' => array(
      drupal_get_path('module', 'nodequery') . '/library-1.js' => array(),
    ),
    'css' => array(
      drupal_get_path('module', 'nodequery') . '/library-2.css' => array(
        'type' => 'file',
        'media' => 'screen',
      ),
    ),
...
function hook_module_implements_alter(&$implementations, $hook) {
  if ($hook == 'rdf_mapping') {
    ...
    $group = $implementations['nodequery'];
    unset($implementations['nodequery']);
    $implementations['nodequery'] = $group;
  }
}
...
function hook_menu_site_status_alter(&$menu_site_status, $path) {
  // Allow access to nodequery/authentication even if site is in offline mode.
  if ($menu_site_status == MENU_SITE_OFFLINE && user_is_anonymous() && $path == 'nodequery/authentication') {
